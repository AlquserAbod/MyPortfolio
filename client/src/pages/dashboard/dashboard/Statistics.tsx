  ...
  useEffect(() => {
    const fetchStatistics = async () => {
      try {
        ...
        setHasStatistics(
          categoriesResponse.data.totalCount > 0 ||
          certificatesResponse.data.totalCount > 0 ||
          skillsResponse.data.totalCount > 0 ||
          projectsResponse.data.totalCount > 0 ||
          usersResponse.data.totalCount > 0 ||
          contactsResponse.data.totalCount > 0
        );
        setLoading(false);
      } catch (error) {
                ...
                {statistics.certificates.latestCertificate && (
                  <Box mt={2}>
                    ...
                    <Typography variant="body2">
                      from "{statistics.certificates.latestCertificate.from}" Taken on : (
                      {new Date(statistics.certificates.latestCertificate.taken_date).toLocaleDateString()})
                    </Typography>
                ...
                {statistics.projects.latestProject && (
                  <Box mt={2}>
                    ...
                    <Typography variant="body2">
                      ...
                      {new Date(statistics.projects.latestProject.createdAt).toLocaleDateString()})
                    </Typography>
                  </Box>
                )}
                <Box mt={2}>
