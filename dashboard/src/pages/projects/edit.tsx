...
export function ProjectEdit() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<Category | null>(projectData?.category || null);
  const { autocompleteProps } = useAutocomplete<Category>({
    resource: "categories",
  });

  const [mainImageURL, setMainImageUrl] = useState<string | null>(projectData?.main_image || null);
  const [projectImagesUrl, setProjectImagesUrl] = useState<string[]>(projectData?.project_images || []);
  ...
  useEffect(() => {    
    if(projectData) {
      ...
      setMainImageUrl(projectData.main_image);
      setProjectImagesUrl(projectData.project_images);
    }
  }, [projectData]);
    ...
    <Edit isLoading={formLoading} saveButtonProps={saveButtonProps}>
      ...
      >
        ...
        <Controller
          name="category"
          control={control}
          defaultValue={projectData?.category || null}
        ...
        <Box mt={2} mb={2}>
          <TinyMCEEditor
            ...
            initialValue={projectData?.description_en || ''}
        ...
        <Box mt={2} mb={2}>
          <TinyMCEEditor
            ...
            initialValue={projectData?.description_tr || ''}
            onChange={(content: string) => {
              const sanitizedContent = DOMPurify.sanitize(content);
        ...
        <Box mt={2} mb={2}>
          <TinyMCEEditor
            ...
            initialValue={projectData?.description_ar || ''}
            onChange={(content: string) => {
              const sanitizedContent = DOMPurify.sanitize(content);
